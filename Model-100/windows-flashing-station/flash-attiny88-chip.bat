...
:try_fuses
for /L %%x in (1, 1, 4) do (
    bin\avrdude -v -pattiny88 -cusbtiny -e -U lfuse:w:0xEE:m -U hfuse:w:0xDD:m -U efuse:w:0xFE:m -U lock:w:0x3F:m && goto try_flash
    echo "Attempt %%x to set fuses failed." >> attiny-log.txt
)
echo "Failed to set fuses after 4 attempts." >> attiny-log.txt
...
echo "Failed to flash ATTiny after 4 attempts." >> attiny-log.txt
